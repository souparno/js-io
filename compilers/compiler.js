...
exports.run = function(args, opts) {
  ...
  if (opts.jsioPath) {
    ...
    J.__jsio.__init__.toString = function() {
      return J.__env.fetch(J.__jsio.__util.buildPath(opts.jsioPath, 'jsio.js'));
    }
  ...
  if (opts['package']) {
    ...
    if (pkgDef.environments) {
      extendArray('environments');
    }
    if (pkgDef.transports) {
      extendArray('transports');
    }
    ...
    if (pkgDef.additional_dependancies) {
      extendArray('additionalDeps', 'additional_dependancies');
    }
    ...
    if (pkgDef.dynamicImports) {
      extendObject('dynamicImports');
    }
  }
  ...
  if (!opts.dynamicImports.ENV) {
    opts.dynamicImports.ENV = null;
  }
